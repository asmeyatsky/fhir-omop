...
export async function renderMappings(root) {
  root.innerHTML = `
    ...
    <!-- Templates -->
    <div class="mb-8">
      <h2 class="text-sm font-semibold text-ink-500 uppercase tracking-wider mb-3">Available Templates</h2>
    ...
    <div class="card">
      <div class="card-header">
        <h3 class="text-sm font-semibold text-ink-900">Active Configurations</h3>
      </div>
  ...
  async function load() {
    try {
      ...
      if (!tpls.length) {
        tplGrid.innerHTML = '<p class="text-ink-500 text-sm col-span-3">No templates available</p>';
      } else {
        tplGrid.innerHTML = tpls.map(t => `
          <div class="card p-4 hover:border-brand/30 transition-colors">
            <div class="flex items-start justify-between mb-2">
              <h4 class="text-sm font-semibold text-ink-900">${escapeHtml(t.name)}</h4>
              <span class="badge badge-teal">${t.field_count} fields</span>
            </div>
            <p class="text-xs text-ink-500 mb-3">${escapeHtml(t.description)}</p>
        ...
        tbl.innerHTML = `
          <table class="data-table">
            ...
            <tbody>
              ${configs.map(c => `
                <tr>
                  <td class="font-medium text-ink-800">${escapeHtml(c.name)}</td>
                  <td class="font-mono text-xs text-ink-500">${c.source_resource}</td>
                  <td class="font-mono text-xs text-ink-500">${c.target_table}</td>
                  <td>${c.field_count}</td>
                  <td>${statusBadge(c.status)}</td>
                  <td class="text-ink-500 text-xs">${formatDateTime(c.created_at)}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
  ...
  document.getElementById('create-mapping-btn').addEventListener('click', () => {
    ...
    showModal(`
      <div class="p-6">
        <h3 class="text-lg font-semibold text-ink-900 mb-4">Create Mapping Configuration</h3>
